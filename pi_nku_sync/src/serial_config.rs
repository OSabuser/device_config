...
impl SerialPortConfig {
    ...
    pub fn new(path_to_scheme: &str) -> Result<Self, String> {
        let smart_serial_parameters = DeviceConfig::create_parameter_list(path_to_scheme)?;

        let serial_name = smart_serial_parameters.get_parameter_value("device")?;
        let serial_baudrate = smart_serial_parameters
